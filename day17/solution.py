...
class Solution:
    ...
    def solve(self, filename, start, inc):
        self.read_file(filename)
        print(f"starting at {start} incrementing by {inc}")
        print(f"state: {self}")
        ...
        while True:
            ...
            if self.output == self.program:
                print(f"{start} got a matching output for i={i}")
                return
            i += inc
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        inc = int(sys.argv[2])
    solution.solve('input.txt', start, inc)
